...
function addRest(){
	document.frm.action= "MadminRestWriteForm";
	document.frm.submit();
}

function addNotice(){
	document.frm.action= "MadminNoticeWriteForm";
	document.frm.submit();
}
...
function go_save(){
	...
	if( document.frm.kind.value==""){  
		...
		document.frm.action = "MadminRestWrite";
		document.frm.submit();
	}
}
...
function go_mod(nseq){
	var url = "MadminNoticeUpdateForm&nseq=" + nseq;
	location.href=url;
...
function go_updateRest(rseq){
	...
	if (document.frm.kind.value == '') {
		...
		if( confirm('수정하시겠습니까?') ){
			 document.frm.action = "MadminRestUpdate?rseq=" + rseq;
			 document.frm.submit();
		}
	}
}
...
function goUpdateMenu(){
	document.frm.action="MadminUpdateMenu";
	document.frm.submit();
}
