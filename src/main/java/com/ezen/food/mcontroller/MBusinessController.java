...
@Controller
public class MBusinessController {
	...
	@RequestMapping("/Mbusiness")
	public String business() {
		return "mobile/business/businessLoginForm";
	}

	@RequestMapping(value = "MbusinessLogin", method = RequestMethod.POST)
	public String businessLogin(@ModelAttribute("dto") @Valid BusinessVO businessvo, BindingResult result,
			HttpServletRequest request, Model model) {

		String url = "mobile/business/businessLoginForm";
		if (result.getFieldError("businessId") != null) {
			...
			if (bvo == null)
				...
				url = "redirect:/MmyRestList";
			}
			return url;
		}
	}

	@RequestMapping("/MbusinessLogout")
	...
	public ModelAndView myRestList(HttpServletRequest request) {
		...
		if (session.getAttribute("loginBusiness") == null)
			mav.setViewName("mobile/business/businessLoginForm");
		else {
			int page = 1;
			...
			mav.setViewName("mobile/business/myRestList");
		}
		return mav;
	}

	@RequestMapping("/MmyRestDetail")
	public ModelAndView my_restdetail(@RequestParam("rseq") int rseq, HttpServletRequest request) {
		...
		if (session.getAttribute("loginBusiness") == null)
			...
			mav.setViewName("mobile/business/myRestDetail");
		}
		return mav;
	}

//08.25
	@RequestMapping("/MmyRestQnaList") // QnA리스트
	public ModelAndView myRestQnaList(HttpServletRequest request) {
		...
		if (session.getAttribute("loginBusiness") == null) {
			...
			mav.addObject("paging", (Paging) result.get("paging"));
			mav.setViewName("mobile/business/myRestQnaList");
		}
		return mav;
	}
	...
	@RequestMapping("/Mboss_qna_detail") // QnA 상세
	public ModelAndView boss_qna_detail(HttpServletRequest request, @RequestParam("qseq") int qseq) {
		...
		mav.setViewName("mobile/business/QnaReply");

		return mav;
	}

	@RequestMapping(value = "MinsertQnaReply", method = RequestMethod.POST) // QnA 등록
	...
	@RequestMapping("/MidCheckFormB") // 중복확인폼
	public String idCheckForm(HttpServletRequest request, Model model, @RequestParam("businessId") String businessId) {
		...
		return "mobile/business/idcheck";
	}



@RequestMapping("/MgetNewCheck")
public ModelAndView getNewMyRestCheck(HttpServletRequest request, @RequestParam("rseq") int rseq) {
	...
	if(session.getAttribute("loginBusiness") == null)
		...
		mav.addObject("paging", (Paging)result.get("paging"));
		mav.setViewName("mobile/business/nRestCheck");
...
@RequestMapping("MgetCompleteCheck")
public ModelAndView getCompleteCheck(HttpServletRequest request, @RequestParam("rseq") int rseq) {
	...
	if(session.getAttribute("loginBusiness") == null)
		...
		mav.addObject("rseq",rseq);
		mav.addObject("paging", (Paging)result.get("paging"));
		mav.setViewName("mobile/business/yRestCheck");
	}
	return mav;
	
}

@RequestMapping("/MrestConfirm")
public String rest_confrim(HttpServletRequest request, @RequestParam("cseq") int cseq,@RequestParam("rseq") int rseq) {
	HttpSession session = request.getSession();
	String url="";
	if(session.getAttribute("loginBusiness")==null){
		url= "mobile/business/businessLoginForm";
	}else {
		
		bs.restConfirm(cseq);
		url="redirect:/MgetNewCheck?rseq="+rseq;
	}
	return url;
}

@RequestMapping("/McancelConfirm")
public String cancel_confrim(HttpServletRequest request, @RequestParam("cseq") int cseq,@RequestParam("rseq") int rseq) {
	...
	if(session.getAttribute("loginBusiness")==null){
		...
		url="redirect:/MgetCompleteCheck?rseq=" + rseq;
	}
	return url;
}
...
@RequestMapping("/MbossJoinForm") // 회원가입폼
public String joinForm(HttpServletRequest request,  Model model) {
	...
	model.addAttribute("businessId", businessId);
	return "mobile/business/bossJoinForm";
}


@RequestMapping("/MjoinBoss") // 사업자 회원가입
public ModelAndView joinBoss(@ModelAttribute("dto") @Valid BusinessVO businessvo, BindingResult result,
		HttpServletRequest request, Model model) {
	ModelAndView mav = new ModelAndView();
	mav.setViewName("mobile/business/bossJoinForm");
	...
	if (result.getFieldError("businessId") != null)
		...
		mav.setViewName("forward:/Mbusiness");
	}
...
@RequestMapping("/MregisterNewRest") // 식당등록폼
public String registerNewRest(HttpServletRequest request,  Model model) {
	return "mobile/business/restInsert";
}
...
public ModelAndView insertNewRest(HttpServletRequest request) {
	...
	if(session.getAttribute("loginBusiness")==null){
		...
		try {
			...
			mav.setViewName("redirect:/MmyRestDetail?rseq=" + rseq);
		} catch (IOException e) {
			e.printStackTrace();
		}
...
@RequestMapping("/MmyRestUpdateForm")
public ModelAndView myRestUpdateForm(@RequestParam("rseq") int rseq, HttpServletRequest request) {
	...
	if(session.getAttribute("loginBusiness")==null) {
		...
		mav.setViewName("mobile/business/myRestUpdate");
	}
	return mav;
		
	}
@RequestMapping("/MmyRestUpdate")
