...
public class Maincontroller {
	...
	public String index(HttpServletRequest request, Model model) {
		...
		ms.getNewRestList(paramMap); //ryn 조건 수정완료
	...
	@RequestMapping("/searchRestList") // 메인 검색
	public ModelAndView searchRestList(HttpServletRequest request) {
		...
		if (session.getAttribute("loginUser") == null) {
			...
			mav.setViewName("restaurant/restKind2");
			HashMap<String, Object> result = ms.searchRestList(page, key); //ryn 조건 수정완료
	...
	public ModelAndView notice(HttpServletRequest request) {
		...
		if (session.getAttribute("loginUser") == null) {
			...
			mav.addObject("key", key);
			mav.addObject("paging", (Paging) result.get("paging"));
			mav.setViewName("restaurant/notice");
		}
		return mav;
	}

	@RequestMapping("/noticeDetail") // 공지사항 상세
	public ModelAndView noticeDetail(@RequestParam("nseq") int nseq, HttpServletRequest request) {
		...
		if (session.getAttribute("loginUser") == null) {
			...
			mav.setViewName("restaurant/noticeDetail");
		}
		return mav;
	}

	@RequestMapping("/category") // 카테고리
	public String catagory(Model model, HttpServletRequest request, @RequestParam("kind") String kind) {
		...
		ms.getKindList(paramMap); //ryn 조건 수정완료 
		...
		return "restaurant/restKind";
	}

	@RequestMapping("/detailForm") // 식당 상세 페이지
	public ModelAndView detailForm(Model model, HttpServletRequest request, @RequestParam("rseq") int rseq,
		...
		mav.setViewName("detail/detail2");
		return mav;
	}

	@RequestMapping("/addWish") // 위시리스트 추가
	public String addWish(Model model, HttpServletRequest request, @RequestParam("rseq") int rseq,
		...
		if (mvo == null) {
			...
			url = "redirect:/detailForm?rseq=" + rseq + "&kind=" + kind;
		}
		return url;
	}

	@RequestMapping("/removeWish") // 위시리스트 삭제
	...
	@RequestMapping("/reserveForm") // 식당 예약폼 이동
	...
	@RequestMapping("/reserveInsert") // 식당 예약
	public ModelAndView reserveForm(@ModelAttribute("dto") @Valid CheckRestVO checkrestvo, BindingResult result,
		...
		if (mvo == null) {
			...
			if (result.getFieldError("c_checkdate") != null)
				...
				mav.setViewName("redirect:/myCheck");
			}
		}
	...
	@RequestMapping("/reviewSubmit") // 리뷰 등록
	public ModelAndView reviewSubmit(@ModelAttribute("dto") @Valid ReviewVO reviewvo, BindingResult result,
		...
		if (mvo == null) {
			...
			mav.setViewName("forward:/detailForm?rseq=" + rseq + "&kind=" + kind);
			if (result.getFieldError("rating1") != null)
				...
				mav.setViewName("redirect:/detailForm?rseq=" + rseq + "&kind=" + kind);
			}
		}
	...
	@RequestMapping("/editReview") // 리뷰 수정 페이지 이동
	...
	public ModelAndView reviewUpdate(@ModelAttribute("dto") @Valid ReviewVO reviewvo, BindingResult result,
		...
		ModelAndView mav = new ModelAndView();
		mav.setViewName("forward:/editReview?rwseq=" + rwseq);
		if (result.getFieldError("rating1") != null)
			model.addAttribute("message", result.getFieldError("rating1").getDefaultMessage());
		else if (result.getFieldError("content") != null)
			model.addAttribute("message", result.getFieldError("content").getDefaultMessage());
			...
			mav.setViewName("detail/result");
		}
		return mav;
	}

	@RequestMapping("/reviewDelete") // 리뷰 삭제
	...
	@RequestMapping("/moreReview") // 리뷰 페이지
	public ModelAndView moreReview(HttpServletRequest request, Model model, @RequestParam("rseq") String rseq) {
		...
		mav.setViewName("detail/reviews");
		return mav;
	}

	@RequestMapping("/bestrest") // 베스트 식당
	public String bestrest(Model model, HttpServletRequest request, @RequestParam("kind") String kind) {
		...
		return "restaurant/bestRest";
	}

	@RequestMapping("/restQna") // 식당 QnA
	public String restQna(Model model, HttpServletRequest request, @RequestParam("rseq") int rseq,
		...
		return "restaurant/qnaList";
	}

	@RequestMapping("/writeQna") // QnA 등록페이지
	public ModelAndView writeQna(Model model, HttpServletRequest request, @RequestParam("rseq") int rseq,
		...
		if (mvo == null) {
			...
			mav.setViewName("restaurant/writeQna");
		}
		return mav;
	}

	@RequestMapping(value="qnaWrite", method = RequestMethod.POST) //QnA 등록
	public String qnaWrite(@ModelAttribute("dto") @Valid QnaVO qnavo, BindingResult result, 
		...
		if(session.getAttribute("loginUser")==null){
			...
			if(result.getFieldError("subject")!=null)model.addAttribute("message",result.getFieldError("subject").getDefaultMessage());
				...
				ms.insertQna(qnavo);
				url = "redirect:/restQna?rseq="+qnavo.getRseq()+"&r_name="+r_name;
			}
		}
		return url;
	}
	
	@RequestMapping("/qnaDelete") // QnA 삭제
