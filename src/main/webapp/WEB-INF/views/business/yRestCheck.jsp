...
<table id="yRestCheck">
	...
	<c:forEach items="${CompleteCheck}" var="ComCheckVO">
		<tr>
			...
			<td><input type="button" value="예약취소" onClick="location.href='cancelConfirm?cseq=${ComCheckVO.cseq}&rseq=${ComCheckVO.rseq} '"></td>
		</tr>
	</c:forEach>
...
<div id="paging" align="center" style="font-size:110%; font-weight:bold;">
	      <c:if test="${paging.prev}">
	         <a href="getCompleteCheck?rseq=${rseq }&pageY=${paging.beginPage-1}">◀</a>&nbsp;
	      </c:if>
	      <c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
	         <c:choose>
	            ...
	            <c:otherwise>
	               <a href="getCompleteCheck?rseq=${rseq }&pageY=${index}">${index}</a>&nbsp;
	            </c:otherwise>
	         </c:choose>
	      </c:forEach>
	      <c:if test="${paging.next}">
	         <a href="getCompleteCheck?rseq=${rseq }&pageY=${paging.endPage+1}">▶</a>&nbsp;
	      </c:if>
