...
<table id="restList">
	...
	<c:forEach items="${qnaList}" var="qna">
		<tr height="50">
			...
			<td><a href="boss_qna_detail?qseq=${qna.qseq}">${qna.subject }</a></td>
			<td>${qna.id }</td>
...
<div id="paging" align="center"
	...
	<c:if test="${paging.prev}">
		<a href="myRestQnaList?page=${paging.beginPage-1}">◀</a>&nbsp;
	      </c:if>
	<c:forEach begin="${paging.beginPage}" end="${paging.endPage}"
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="myRestQnaList?page=${index}&key=${key}">${index}</a>&nbsp;
	            </c:otherwise>
		</c:choose>
	</c:forEach>
	<c:if test="${paging.next}">
		<a href="myRestQnaList?page=${paging.endPage+1}">▶</a>&nbsp;
	      </c:if>
