...
<table id="nRestCheck">
	...
	<c:forEach items="${NewCheck}" var="newCheckVO">
		<tr>
			...
			<td height="50"><input id="completeButton" type="button" value="예약확정" onClick="location.href='MrestConfirm?cseq=${newCheckVO.cseq}&rseq=${newCheckVO.rseq} '"></td>
		</tr>
	</c:forEach>
</table>
<br>
...
<div id="paging" align="center" style="font-size:110%; font-weight:bold;">
	      <c:if test="${paging.prev}">
	         <a href="MgetNewCheck?rseq=${rseq }&pageN=${paging.beginPage-1}">◀</a>&nbsp;
	      </c:if>
	      <c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
	         <c:choose>
	            ...
	            <c:otherwise>
	               <a href="MgetNewCheck?rseq=${rseq }&pageN=${index}">${index}</a>&nbsp;
	            </c:otherwise>
	         </c:choose>
	      </c:forEach>
	      <c:if test="${paging.next}">
	         <a href="MgetNewCheck?rseq=${rseq }&pageN=${paging.endPage+1}">▶</a>&nbsp;
	      </c:if>
