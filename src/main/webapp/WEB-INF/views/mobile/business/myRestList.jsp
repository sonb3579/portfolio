	...
	<table id="restList">
		...
		<c:forEach items="${myRestList}" var="RestVO">
			...
			<td height="50" align="center">
					<a href="#" onclick="location.href='MmyRestDetail?rseq=${RestVO.rseq}'">${RestVO.r_name }</a></td>
		...
		<div id="paging" align="center" style="font-size:110%; font-weight:bold;">
	      <c:if test="${paging.prev}">
	         <a href="MmyRestList?page=${paging.beginPage-1}">◀</a>&nbsp;
	      </c:if>
	      <c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
	         <c:choose>
	            ...
	            <c:otherwise>
	               <a href="MmyRestList?page=${index}&key=${key}">${index}</a>&nbsp;
	            </c:otherwise>
	         </c:choose>
	      </c:forEach>
	      <c:if test="${paging.next}">
	         <a href="MmyRestList?page=${paging.endPage+1}">▶</a>&nbsp;
	      </c:if>
