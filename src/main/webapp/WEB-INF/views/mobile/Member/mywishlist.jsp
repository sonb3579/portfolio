	...
	<div id="rightmypage1">
		<div id ="reserveBox">
					...
					<div id="mypage_box">
						<div id="history_img">
							<img src="/images/RestImages/${WishVO.m_image }"
									onClick="location.href='MdetailForm?rseq=${WishVO.rseq}&kind=${WishVO.r_kind}'"/>
						</div>
						<div id="history_content">
							 <h3 onClick="location.href='MdetailForm?rseq=${WishVO.rseq}&kind=${WishVO.r_kind}'"> ${WishVO.r_name} </h3><br>
							평점 : ${WishVO.rating/2}
			...
			<c:if test="${paging.prev}">
				<a href="MmyWishlist?page=${paging.beginPage-1}">◀</a>&nbsp;
			</c:if>
				<c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
					<c:choose>
						...
						<c:otherwise>
							<a href="MmyWishlist?page=${index}">${index}</a>&nbsp;
						</c:otherwise>
					</c:choose>
				</c:forEach>
			<c:if test="${paging.next}">
				<a href="MmyWishlist?page=${paging.endPage+1}">▶</a>&nbsp;
			</c:if>
		</div>
		</div>
