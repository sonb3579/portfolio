			...
			<table id="searchBox">
				<tr>
					<td width="642"><input type="text" name="key" value="${key}">
						...
						<input class="btn" type="button" name="btn_write" value="공지사항등록"  onClick="addNotice();">
					</td>
				</tr>
			</table>	
	</form>
		...
		<c:forEach items="${noticeList}" var="noticeVO">
			<tr><td height="50" align="center" >${noticeVO.nseq}</td>
				<td height="50" style="text-align:center;">
					<a href="adminNoticeDetail?nseq=${noticeVO.nseq}">${noticeVO.subject}</a>
				</td>
	...
	<div id="paging" align="center" style="font-size:110%; font-weight:bold;">
		...
		<c:forEach begin="${paging.beginPage}" end="${paging.endPage}" var="index">
			<c:choose>
				...
				<c:otherwise>
					<a href="adminNoticeList?page=${index}&key=${key}">${index}</a>&nbsp;
				</c:otherwise>
			</c:choose>
		</c:forEach>
